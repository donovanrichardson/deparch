...
export default {
    ...
    created: async function () {
        ...
        const url = `http://localhost:3001/timetable?agency=${escape(dquery.agency)}&date=${dquery.date}&route=${escape(dquery.route)}&origin=${escape(dquery.origin)}&dest=${escape(dquery.dest)}`
        const {
            data
        } = await axios.get(url)
        this.data = data;
